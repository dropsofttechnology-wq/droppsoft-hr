...
import {
  ...
  deactivateSalaryAdvanceRequest
...
import './SalaryAdvanceManagement.css'
...
const SalaryAdvanceManagement = () => {
  ...
  const canApproveAdvance = hasPermission(user, 'salary_advance_approval')
  const canManageAdvance = hasPermission(user, 'salary_advance_management')
  const canDeactivateAdvance = hasPermission(user, 'salary_advance_deactivate')
  ...
  const load = async () => {
    ...
    try {
      setLoading(true)
      const data = await getSalaryAdvanceRequests(currentCompany.$id, {
  ...
  useEffect(() => {
    ...
    const rowEl = document.getElementById(`advance-row-${highlightedRequestId}`)
    if (!rowEl) return
    window.setTimeout(() => {
      rowEl.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' })
      if (lastJumpToastId !== highlightedRequestId) {
        toast.success(`Jumped to salary advance request ${highlightedRequestId}`)
  ...
  const doApproveConfirm = async () => {
    ...
    try {
      ...
      await approveSalaryAdvanceRequest(approveModal.id, 'approved', opts)
      toast.success('Approved — advance split posted to payroll deductions for each month.')
  ...
  const doReject = async (id) => {
    try {
      await approveSalaryAdvanceRequest(id, 'rejected')
  ...
  const saveFormNotesModal = async () => {
    ...
    try {
      await updateSalaryAdvanceRequest(formNotesModal.id, { admin_form_notes: formNotesModal.text })
  ...
  const saveEditModal = async () => {
    ...
    setEditSaving(true)
    try {
      await updateSalaryAdvanceRequest(editModal.id, {
        amount: amt,
  ...
  const doDelete = async (id) => {
    try {
      await deleteSalaryAdvanceRequest(id)
  ...
  const doDeactivate = async (id) => {
    try {
      await deactivateSalaryAdvanceRequest(id)
  ...
  const printForm = (req) => {
    ...
    const approvedLabel = req.approved_at
      ...
      : '—'
    openPrintableForm(
      req.status === 'pending' ? 'Salary advance request' : 'Salary advance approval',
    ...
    return (
      <div className="sar-mgmt-page">
        <div className="alert alert-warning">
          Salary advance management is only available in the desktop (local SQLite) app.
        </div>
      </div>
    )
  }
    ...
    <div className="sar-mgmt-page">
      <div className="page-header">
        ...
        <p className="page-description">
          ...
          first payroll month and instalment count before approving — e.g. an advance dated 15 March deducts from March
            ...
            <tbody>
              {filteredRows.map((r) => (
                ...
                >
                  ...
                  <td>{r.installment_count != null ? r.installment_count : '—'}</td>
                  ...
                  <td>
                    {r.status === 'pending' ? (
                      ...
                      <div className="table-actions">
                        {!canManageAdvance && (r.status === 'approved' || r.status === 'rejected') && (
                          <button type="button" className="action-btn action-btn-edit" onClick={() => openEditModal(r)}>
                            Edit
                          </button>
                        )}
        ...
        >
          <div className="sar-modal" onClick={(e) => e.stopPropagation()}>
            <h2 className="sar-modal-title">
              {editModal.row.status === 'approved' ? 'Edit approved salary advance' : 'Edit salary advance request'}
            </h2>
            ...
            <div className="sar-modal-fields">
              ...
              <label>
                Administrator notes (shown on printed salary advance form)
                <textarea
                  rows={3}
                  value={editModal.admin_form_notes ?? ''}
                  onChange={(e) => setEditModal((m) => ({ ...m, admin_form_notes: e.target.value }))}
                  placeholder="Optional — appears on the printable form"
                />
              </label>
            </div>
            {editModal.row.status === 'approved' && (
              <div className="sar-modal-preview">
                ...
                <p className="sar-modal-preview-meta">
                  Starting <code>{editInstallmentPreview.start}</code> — {editInstallmentPreview.n} month(s), total KES{' '}
      ...
      {formNotesModal ? (
        ...
        >
          <div className="sar-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="sar-modal-lead">
              These notes appear on the printable salary advance form for this request (not the employee&apos;s purpose /
              reason).
            </p>
            <textarea
              rows={6}
      ...
      {approveModal ? (
        <div className="sar-modal-overlay" role="dialog" aria-modal="true" onClick={() => !approveLoading && setApproveModal(null)}>
          <div className="sar-modal" onClick={(e) => e.stopPropagation()}>
            <h2 className="sar-modal-title">Approve salary advance</h2>
            <p className="sar-modal-lead">
              Confirm how the advance is split across payroll months. Amounts are added to each month&apos;s{' '}
              <strong>advance deduction</strong> (net pay) — same data payroll and reports use.
            </p>
            <div className="sar-modal-fields">
              <label>
            ...
            <div className="sar-modal-preview">
              ...
              <p className="sar-modal-preview-meta">
                Starting <code>{installmentPreview.start}</code> — {installmentPreview.n} month(s), total KES{' '}
            ...
            <div className="sar-modal-actions">
              ...
              <button type="button" className="btn-primary" disabled={approveLoading} onClick={doApproveConfirm}>
                {approveLoading ? 'Posting…' : 'Approve & post to payroll'}
              </button>
            </div>
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        message="Reject this salary advance request?"
        confirmLabel="Reject"
        danger
      ...
      <ConfirmDialog
        ...
        title="Deactivate salary advance"
        message="Reverse posted payroll deductions for this advance and mark it voided?"
