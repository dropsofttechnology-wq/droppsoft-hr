...
import { mapShoppingRow } from '../utils/rowMappers.js'
import {
  installmentPlanMatchesTotal,
  parseInstallmentPlan,
  splitMoneyIntoInstallments
} from '../utils/moneySplit.js'
import { mergeAddShoppingDeduction } from '../utils/mergeEmployeeDeductionAdvance.js'

/**
 * @param {import('better-sqlite3').Database} db
 */
export function createShoppingRoutes(db) {
  const r = Router()
  const auth = requireLocalUser(db)

  function normalizeItemLines(rawLines) {
    ...
    for (const raw of rawLines) {
      ...
      if (!itemCode || !Number.isFinite(qty) || qty <= 0 || !Number.isFinite(unitPrice) || unitPrice < 0) continue
  ...
  function buildInstallmentSchedule(row) {
    ...
    if (customPlan.length && !installmentPlanMatchesTotal(customPlan, amount)) {
      throw new Error('Custom installment amounts must add up exactly to the shopping amount')
    }
  ...
  /** First payroll month that receives a deduction slice (e.g. shopping taken 15 Mar â†’ March payroll). */
  ...
  /** Undo payroll deductions posted for this approved shopping (same schedule as post). */
  function reverseApprovedAdvanceFromPayroll(row) {
    ...
    slices.forEach((slice, i) => {
      ...
      if (!existing) return
      const prevAdv = Number(existing.shopping_amount) || 0
  ...
  function repostApprovedAdvanceOpenPeriodsOnly(previousRow, nextRow) {
    ...
    if (remaining < 0) {
      throw new Error(
        `New shopping amount is lower than already posted closed installments (${paidClosedTotal.toFixed(2)}).`
      )
    }
    ...
    oldOpen.forEach((item) => {
      ...
      const notes = String(existing.notes || '')
        ...
        .join('\n')
        .trim()
      db.prepare('UPDATE employee_deductions SET shopping_amount = ?, notes = ?, updated_at = ? WHERE id = ?').run(
        newAdv,
        notes,
    ...
    if (nextPlan.length) {
      ...
      if (!installmentPlanMatchesTotal(openSlices, remaining)) {
        throw new Error(
          'For approved shoppings with closed periods, custom installment amounts must equal only the remaining balance.'
        )
      }
    ...
    const ref = `Shopping ${nextRow.id.slice(0, 8)}â€¦`
  ...
  r.get('/requests', auth, (req, res) => {
    try {
      ...
      res.json(rows.map(mapShoppingRow))
    } catch (e) {
  ...
  r.post('/requests', auth, (req, res) => {
    try {
      ...
      const itemLines = normalizeItemLines(payload.item_lines)
      const itemLinesTotal = Math.round(itemLines.reduce((sum, line) => sum + (Number(line.total) || 0), 0) * 100) / 100
      const amountInput = Number.parseFloat(String(payload.amount ?? ''))
      const amount = itemLines.length ? itemLinesTotal : amountInput
      ...
      if (payload.for_period != null && String(payload.for_period).trim() !== '') {
        if (!roleHasPermission(db, role, 'salary_shopping_management')) {
          return res.status(403).json({ error: 'Only managers and admins can set payroll month for an shopping' })
        }
        const fp = String(payload.for_period).trim().slice(0, 7)
        if (!/^\d{4}-\d{2}$/.test(fp)) {
      ...
      if (payload.installment_plan != null && String(payload.installment_plan).trim() !== '') {
        ...
        if (!installmentPlanMatchesTotal(parsed, amount)) {
          return res
            .status(400)
            .json({ error: 'Custom installment amounts must add up exactly to the shopping amount' })
        }
      ...
      db.prepare(
        `INSERT INTO shopping_requests (
          ...
          installment_plan, item_lines_json, requested_at, requested_by, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?)`
      ).run(
        id,
        companyId,
        ...
        installmentPlan,
        itemLines.length ? JSON.stringify(itemLines) : null,
        now,
        req.userId,
        now,
        now
      ...
      const row = db
        ...
        .get(id)
      res.status(201).json(mapShoppingRow(row))
    } catch (e) {
  ...
  r.post('/requests/:id/approve', auth, (req, res) => {
    try {
      const role = getUserRole(db, req.userId)
      if (!roleHasPermission(db, role, 'salary_shopping_approval')) {
      ...
      const result = runInTransaction(db, () => {
        let row = db.prepare('SELECT * FROM shopping_requests WHERE id = ?').get(requestId)
        ...
        if (row.requested_by && String(row.requested_by) === String(req.userId)) {
          throw new Error('Maker-checker rule: the requester cannot approve/reject their own shopping request.')
        }
        if (!row.requested_by) {
        ...
        if (decision === 'approved') {
          ...
          row = db.prepare('SELECT * FROM shopping_requests WHERE id = ?').get(requestId)
      ...
      res.json(mapShoppingRow(result))
    } catch (e) {
      res.status(400).json({ error: e.message })
    }
  })
  ...
  r.post('/requests/:id/deactivate', auth, (req, res) => {
    try {
      if (getUserRole(db, req.userId) !== 'super_admin') {
        return res.status(403).json({ error: 'Super admin only' })
      }
      const requestId = req.params.id
      const now = new Date().toISOString()
      const result = runInTransaction(db, () => {
        const row = db.prepare('SELECT * FROM shopping_requests WHERE id = ?').get(requestId)
        if (!row) throw new Error('Not found')
        if (row.status !== 'approved') {
          throw new Error('Only approved shoppings can be deactivated')
        }
        reverseApprovedAdvanceFromPayroll(row)
        db.prepare(
          `UPDATE shopping_requests SET status = 'voided', updated_at = ? WHERE id = ?`
        ).run(now, requestId)
        db.prepare(
  ...
  r.patch('/requests/:id', auth, (req, res) => {
    try {
      ...
      if (row.status === 'voided') {
        return res.status(400).json({ error: 'Voided shoppings cannot be edited' })
      }
      ...
      const result = runInTransaction(db, () => {
        ...
        const itemLines = body.item_lines !== undefined ? normalizeItemLines(body.item_lines) : null
        if (itemLines && itemLines.length) {
          amount = Math.round(itemLines.reduce((sum, line) => sum + (Number(line.total) || 0), 0) * 100) / 100
        ...
        if (body.installment_plan != null) {
          ...
          if (parsedExisting.length && !installmentPlanMatchesTotal(parsedExisting, amount)) {
            throw new Error(
              'Amount changed: update custom installment amounts so they add up exactly to the new shopping amount'
            )
          }
        }
        ...
        db.prepare(
          `UPDATE shopping_requests
           SET amount = ?, reason = ?, repayment_period = ?, for_period = ?, application_date = ?, installment_count = ?, installment_plan = ?, item_lines_json = ?, admin_form_notes = ?, updated_at = ?
           WHERE id = ?`
        ).run(
          amount,
          reason,
          ...
          installmentCount ?? null,
          installmentPlan || null,
          itemLines ? (itemLines.length ? JSON.stringify(itemLines) : null) : row.item_lines_json || null,
          adminFormNotes,
          now,
          requestId
        )

        let updated = db.prepare('SELECT * FROM shopping_requests WHERE id = ?').get(requestId)
  ...
  r.delete('/requests/:id', auth, (req, res) => {
    try {
      ...
      const row = db.prepare('SELECT * FROM shopping_requests WHERE id = ?').get(req.params.id)
      if (!row) return res.status(404).json({ error: 'Not found' })
      if (row.status !== 'pending') {
        return res.status(400).json({ error: 'Only pending requests can be deleted' })
      }
      db.prepare('DELETE FROM shopping_requests WHERE id = ?').run(req.params.id)
      res.json({ ok: true })
    } catch (e) {
      res.status(400).json({ error: e.message })
    }
